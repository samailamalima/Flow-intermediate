...
// MalimaToken contract: Implements fungible token on Flow blockchain
pub contract MalimaToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @MalimaToken.Vault
        ...
        // Destroy the vault
        destroy() {
            MalimaToken.totalSupply = MalimaToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            MalimaToken.totalSupply = MalimaToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&MalimaToken.Minter>(/public/Minter, target: /storage/MinterStorage)
